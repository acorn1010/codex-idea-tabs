...
export const ImagePreview = memo(function ImagePreview({ path, alt = 'Image' }: { path: string; alt?: string }) {
  ...
  return <span className="my-2 inline-flex max-w-full flex-col gap-1">
    {url ? <button className="max-w-full overflow-hidden rounded-lg border border-line bg-composer hover:border-accent active:border-ink active:brightness-90" onClick={() => setExpanded(true)} aria-label={`Preview ${alt}`}><img src={url} alt={alt} loading="lazy" onError={() => { setUrl(''); setError('This image could not be decoded. Open it in the editor.'); }} className="max-h-72 max-w-full object-contain" /></button> : <span className="rounded-lg border border-line px-4 py-3 text-muted">{error || 'Loading image…'}</span>}
...
export const Markdown = memo(function Markdown({ text }: { text: string }) {
  return <ReactMarkdown remarkPlugins={[remarkGfm]} urlTransform={(url) => url} components={{
    ...
    pre: ({ children }) => <pre className="my-3 overflow-x-auto rounded-lg border border-line bg-composer p-3 text-xs leading-relaxed">{children}</pre>,
    ...
    table: ({ children }) => <div className="my-3 overflow-x-auto rounded border border-line"><table className="w-full border-collapse text-left text-xs">{children}</table></div>,
